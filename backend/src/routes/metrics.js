...
router.post('/', async (req, res) => {
  try {
    const { hostname, ip, mac, username, department, os, cpu_model, ram_total, gpu,
            cpu, ram, disk, network, processes, connections } = req.body;
    ...
    const deviceData = { hostname, ip, mac, username, department, os, cpu_model,
                         ram_total, gpu, status: 'online', last_seen: now };
    if (existing) {
    ...
    if (Array.isArray(processes)) {
      ...
      if (processes.length > 0) {
        await db.processes.insert(
          processes.slice(0, 30).map(p => ({ ...p, device_id: hostname, timestamp: now }))
        );
      }
    }
    ...
    if (Array.isArray(connections)) {
      ...
      if (connections.length > 0) {
        await db.connections.insert(
          connections.slice(0, 50).map(c => ({ ...c, device_id: hostname, timestamp: now }))
        );
      }
    }
    ...
    // Broadcast real-time
    ws.broadcastAll({
      type: 'metrics_update', deviceId: hostname,
      data: { hostname, ip, username, department,
              cpu_percent: cpu?.percent, ram_percent: ram?.percent,
              disk_percent: disk?.percent, net_download: network?.download_mbps,
              net_upload: network?.upload_mbps, ping_ms: network?.ping_ms,
              packet_loss: network?.packet_loss, status: 'online', last_seen: now },
    });
...
router.get('/:id/latest', async (req, res) => {
  try {
    const metrics = await db.metrics.find({ device_id: req.params.id }).sort({ timestamp: -1 }).limit(1);
    if (!metrics.length) return res.status(404).json({ error: 'Tidak ada data' });
...
router.get('/:id', async (req, res) => {
  try {
    ...
    res.json({ deviceId: req.params.id, range: req.query.range || '1h', count: metrics.length, metrics });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
