...
function getStoredSession(): { token: string; user: AuthUser } | null {
  try {
    const token =
      localStorage.getItem("netwatch_token") ||
      sessionStorage.getItem("netwatch_token");
    const userStr =
      localStorage.getItem("netwatch_user") ||
      sessionStorage.getItem("netwatch_user");
...
export default function App() {
  ...
  const loadDashboard = useCallback(async () => {
    try {
      ...
      devicesRes.devices.forEach((d) => {
        newMetrics[d.hostname] = mapMetric(d);
      });
      setPcs(newPcs);
      setMetrics(newMetrics);
      setTotalAlerts(summaryRes.activeAlerts);
      setError(null);
      setHistory((prev) => {
        const next = { ...prev };
        newPcs.forEach((pc) => {
          if (!next[pc.id]) next[pc.id] = [];
        });
        return next;
      });
    } catch (e) {
  ...
  const loadDetail = useCallback(async (pc: PC) => {
    ...
    try {
      ...
      setHistory((prev) => ({
        ...prev,
        [pc.id]: histRes.metrics.map((m) => mapMetricRaw(m, pc.id)),
      }));
      setDetail({
        processes: detailRes.processes.map(mapProcess),
        connections: detailRes.connections,
        alerts: detailRes.alerts.map(mapAlert),
        logs: logsRes.logs.map(mapLog),
        loading: false,
      });
      if (detailRes.latestMetric)
        setMetrics((prev) => ({
          ...prev,
          [pc.id]: mapMetricRaw(detailRes.latestMetric, pc.id),
        }));
    } catch (e) {
  ...
  const handleSelectPC = useCallback(
    (pc: PC) => {
      setSelectedPC(pc);
      loadDetail(pc);
      wsClient.subscribeDevice(pc.id);
    },
    [loadDetail],
  );
    ...
    const unsub = wsClient.on((event) => {
      if (event.type === "metrics_update") {
        ...
        const m: PCMetrics = {
          ...
          ramUsage: data.ram_percent || 0,
          diskUsage: data.disk_percent || 0,
          diskRead: 0,
          diskWrite: 0,
          ping: data.ping_ms || 0,
          bandwidthDown: data.net_download || 0,
          bandwidthUp: data.net_upload || 0,
        ...
        setHistory((prev) => ({
          ...prev,
          [deviceId]: [...(prev[deviceId] || []).slice(-59), m],
        }));
        setPcs((prev) => {
          const exists = prev.find((p) => p.id === deviceId);
          if (exists)
            return prev.map((p) =>
              p.id === deviceId ? { ...p, status: data.status || p.status } : p,
            );
          return [
            ...prev,
            {
        ...
        setPcs((prev) =>
          prev.map((p) =>
            p.id === event.deviceId
              ? {
                  ...p,
                  status: event.severity === "critical" ? "warning" : p.status,
                }
              : p,
          ),
        );
      ...
      if (event.type === "device_offline")
        setPcs((prev) =>
          prev.map((p) =>
            p.id === event.deviceId
              ? { ...p, status: "offline", lastOnline: Date.now() }
              : p,
          ),
        );
    });
  ...
  useEffect(() => {
    ...
    pollRef.current = setInterval(() => {
      if (!wsClient.isConnected) loadDashboard();
    }, 30_000);
    return () => {
    ...
    return (
      <div className="min-h-screen bg-[#141518] flex items-center justify-center p-6">
        <div className="text-center space-y-4 max-w-sm">
          <div className="w-12 h-12 bg-red-500/20 rounded-full flex items-center justify-center mx-auto text-red-400 text-xl">
            !
          </div>
  ...
  return (
    <div className="min-h-screen bg-[#141518] font-sans selection:bg-blue-500/30">
      ...
      <div className="fixed top-3 right-4 z-50 flex items-center gap-3">
        ...
        <button
          onClick={handleLogout}
          className="text-xs text-gray-500 hover:text-red-400 transition-colors border border-gray-700 hover:border-red-500/50 rounded-lg px-3 py-1.5"
        >
          Logout
        </button>
      </div>
      ...
      <div className="fixed bottom-4 right-4 z-50 flex items-center gap-2 bg-[#1E1F23] border border-gray-700 rounded-full px-3 py-1.5 text-xs text-gray-400">
        <div
          className={`w-2 h-2 rounded-full ${wsClient.isConnected ? "bg-emerald-500 animate-pulse" : "bg-red-500"}`}
        />
      ...
      {selectedPC ? (
        <PCDetail
          pc={selectedPC}
          metrics={metrics[selectedPC.id]}
          history={history[selectedPC.id] || []}
          alerts={detail.alerts}
          logs={detail.logs}
          processes={detail.processes}
          connections={detail.connections} // ← tambah ini
